...
class MovieExceptionHandler {
    ...
    @ExceptionHandler(OnActorServiceUnavailable::class)
    fun handleOnActorServiceUnavailable(): ResponseEntity<ErrorResponse> {
        val body = ErrorResponse(
            ACTOR_SERVICE_UNAVAILABLE.name,
            ACTOR_SERVICE_UNAVAILABLE.wording,
            "The Actor component is currently unavailable. Please try again later."
        )
        return ResponseEntity(body, SERVICE_UNAVAILABLE)
    }

    @ExceptionHandler(OnSummaryNotFound::class)
    fun handleOnSummaryNotFound(ex: OnSummaryNotFound): ResponseEntity<ErrorResponse> {
        val body = ErrorResponse(
            SUMMARY_NOT_FOUND.name,
            SUMMARY_NOT_FOUND.wording,
            "No summary found for the movie titled '${ex.title}' released in ${ex.year}."
        )
